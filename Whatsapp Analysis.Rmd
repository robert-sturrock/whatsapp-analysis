...
response_summary_word(tmp, "Isabel Lachenauer", type = 2, word="sex")
...
# Note: the response matrix only works if the matrix is full 
# to solve this we might be able 
...
Note that this response matrix function only works if the entire response matrix is full (ie everyone said every word to each person).


We want to fix this so that the function works regardless of the completeness of the matrix. 

```{r}



# try and just accomplish it once
response_summary_word(tmp, "Liam Kirwin", 1, "sex")
...
#select sender 
  test <- tmp 
  test$indicator <- 0 
  test[test$sender == "Liam Kirwin" & grepl("sex", test$message),]$indicator <- 1 
  test$indicator <- lag(test$indicator)

#calculate total messages by sender 

test %>% group_by(sender) %>% summarise(total_responses = sum(na.omit(indicator)))
  
test3 <- test %>% group_by(sender) %>% summarise(other_var = 0) %>% left_join(test2) %>% mutate( total_responses = ifelse(is.na(total_responses), other_var, total_responses))
...
* do analysis of messages by number of words 
* do network and table for an individual word 
* topic analysis 
